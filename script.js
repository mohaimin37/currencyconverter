...
const selt1 = document.querySelector(".selt1")
const opt = document.getElementById("opt")
const opti =document.getElementById("opti")
...
array.forEach((elem)=>{
    const anas = document.createElement("option")
   anas.textContent =`${elem.code}  ${elem.name}`
   anas.value = elem.code

    opt.appendChild(anas)
    opt.value = "INR"
    

})
array.forEach((name)=>{
    const abdul = document.createElement("option")
   abdul.value = name.code

    abdul.innerHTML = `${name.code} ${name.name}`
    opti.appendChild(abdul)
    opti.value = "USD"
})
const sell = async()=>{
    let amount = selt.value
    const country = opt.value
    const country1 = opti.value
    const anas =await fetch(`https://api.exchangerate-api.com/v4/latest/${country}`)
    const abd = await anas.json()
    console.log(country,country1);
    selt1.value = amount*abd.rates[country1]
    h4.textContent =`${amount}${country}=${selt1.value}${country1}`
   

}
selt.addEventListener('input',()=>{
    sell()
